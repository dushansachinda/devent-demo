...
service / on new http:Listener(8080) {
    ...
    resource function put orders/[string orderId](@http:Payload OrderRequest orderRequest) returns Order|error {
        if !orders.hasKey(orderId) {
            return error("Order not found");
        }
...
listener agent:Listener pizzaAiListener = new (listenOn = check http:getDefaultListener());

service /pizzaAi on pizzaAiListener {
